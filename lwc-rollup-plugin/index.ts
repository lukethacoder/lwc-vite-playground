...
function isImplicitCssImport(importee: string, importer: string): boolean {
  return (
    ...
    (path.basename(importee, '.css') === path.basename(importer, '.html') ||
      path.basename(importee, '.scoped.css') ===
        path.basename(importer, '.html'))
  )
}
...
function transformWarningToRollupLog(
  ...
  if (location) {
    ...
    if (
      ...
      typeof location.length === 'number'
    ) {
      result.frame = src.substring(
        location.start,
    ...
    resolveId(importee, importer) {
      if (importer) {
        ...
        if (importee.startsWith('.') || importee.startsWith('/')) {
          ...
          try {
            const { entry, specifier, type } = resolveModule(
              importee,
              importer,
              {
                modules,
